...
#define VIXDISKLIB_HWVERSION_CURRENT             VIXDISKLIB_HWVERSION_ESX65

// Minimal number of sectors per chunk
#define VIXDISKLIB_MIN_CHUNK_SIZE               (64 * 2)          // 64K
// Maximal number of sectors per chunk
#define VIXDISKLIB_MAX_CHUNK_SIZE               (64 * 2 * 1024)   // 64M
...
/**
 * Get the blocks allocated.
 * @param diskHandle [in] Handle to an open virtual disk.
 * @param startSector [in] Absolute offset.
 * @param numSectors [in] Number of sectors to query.
 * @param chunkSize [in] Minimum number of sectors covered by a chunk
 *                       containing data.
 * @param blockList [out] Buffer contains a VixDiskLibBlockList.
 * @return VIX_OK if success, suitable VIX error code otherwise.
 */
VixError
...
/**
 * Frees memory allocated in VixDiskLib_QueryAllocatedBlocks.
 * @param blockList [in] block list to be freed.
 */
VixError
